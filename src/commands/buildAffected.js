...
async function buildAffectedChartsCommand(options) {
  try {
    ...
    const changedFiles = shell
      .exec(`git diff --name-only ${gitReference}`, { silent: true })
      .stdout.split("\n");
    console.log(
      chalk.yellow(`Detected changed files:
  ${changedFiles.join("\n")}`)
    );

    let affectedCharts = charts.filter((chartPath) =>
    ...
    console.log(
      chalk.green("✔ Affected charts and dependencies built successfully!")
    );
  } catch (error) {
